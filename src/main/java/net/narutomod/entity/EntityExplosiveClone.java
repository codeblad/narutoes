...
public class EntityExplosiveClone extends ElementsNarutomodMod.ModElement {
	...
	public static class EC extends EntityClone.Base implements ItemJutsu.IJutsu {
		...
		public EC(EntityLivingBase user) {
			...
			this.getEntityAttribute(SharedMonsterAttributes.MAX_HEALTH).setBaseValue(user.getMaxHealth()*defense*0.15);
			this.getEntityAttribute(SharedMonsterAttributes.MOVEMENT_SPEED).setBaseValue(0.5d + ProcedureUtils.getModifiedSpeed(user));
