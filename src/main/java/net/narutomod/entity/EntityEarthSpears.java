...
public class EntityEarthSpears extends ElementsNarutomodMod.ModElement { 
	...
	public static class EC extends EntitySpike.Base implements ItemJutsu.IJutsu { 
		...
		private final int riseTime = 4; 
		private final int peakTime = 4; 
		private final int fallTime = 6; 
		...
		public void onUpdate() { 
			...
			if (this.ticksAlive < this.initialDelay) { 
				...
				this.setPosition(this.posX, this.targetY - this.riseHeight + this.riseHeight * progress, this.posZ); 
				...
				this.setPosition(this.posX, this.targetY - this.riseHeight * progress, this.posZ); 
			...
			if ((this.ticksAlive == damageTick1 || this.ticksAlive == damageTick3) && this.damageCount < 3) { 
				this.damageCount++; 
				for (EntityLivingBase entity :  
				 this.world.getEntitiesWithinAABB(EntityLivingBase.class, this.getEntityBoundingBox().grow(1d, 0.5d, 1d))) { 
			...
			public boolean createJutsu(ItemStack stack, EntityLivingBase entity, float power) { 
				...
				if (res != null && res.typeOfHit == RayTraceResult.Type.BLOCK) { 
					...
					for (int i = 0; i < Math.round(power); i++) { 
						...
						entity1.damage = 12+5f*(1+4f*(1/100))*ItemJutsu.getDmgMult(entity); 
