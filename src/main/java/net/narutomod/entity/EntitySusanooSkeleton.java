...
public class EntitySusanooSkeleton extends ElementsNarutomodMod.ModElement {
	...
	public static class EntityCustom extends EntitySusanooBase {
		...
		public EntityCustom(EntityLivingBase player, boolean full) {
			super(player);
			this.setSize(2.4F, 2.4F);
			this.getEntityAttribute(SharedMonsterAttributes.ATTACK_DAMAGE).setBaseValue(25+ItemJutsu.getDmgMult(player)*2.25);
			float ratio = 1;
			if (!full) {
				...
				this.getEntityAttribute(SharedMonsterAttributes.MAX_HEALTH).setBaseValue(health*1.5+150);
			}
