	...
	public class ItemJinton extends ElementsNarutomodMod.ModElement {
		...
		private static final int MIN_PLAYER_XP = 40;
		...
		public static class RangedItem extends ItemJutsu.Base {
			...
			private float getMaxUsablePower(EntityLivingBase entity, ItemStack stack) {
				float max = entity instanceof EntityPlayer ? MIN_PLAYER_XP / 10f : 6;
			...
			private float getUsePercent(int timeLeft) {
				...
				float t = Math.min(f / 400f, 1.0f);
				return (float)Math.pow(t, 0.5d);
			}

			private float getXpPowerCap(EntityLivingBase entity) {
				if (entity instanceof EntityPlayer) {
					double xp = PlayerTracker.getBattleXp((EntityPlayer) entity) ;
					if (xp >= 10000d) {
						return 5f;
					} else if (xp >= 7500d) {
						return 4f;
			...
			public float getPower(ItemStack stack, EntityLivingBase entity, int timeLeft) {
				float power = Math.min(this.getUsePercent(timeLeft) * this.getMaxUsablePower(entity, stack), this.getMaxPower(stack, entity));
				return Math.min(power, this.getXpPowerCap(entity));
			}
		...
		public static class EntityBeam extends EntityBeamBase.Base implements ItemJutsu.IJutsu {
			...
			public void onUpdate() {
				...
				if (!this.world.isRemote && this.ticksAlive > this.wait + 40)
	{
					this.setDead();
				}
			}
