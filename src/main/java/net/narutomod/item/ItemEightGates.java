...
public class ItemEightGates extends ElementsNarutomodMod.ModElement {
	...
	public static class Properties {
		...
		public void deActivate(EntityLivingBase entity) {
			...
			if (!entity.world.isRemote && this.gate > 1 && (!(entity instanceof EntityPlayer) || !((EntityPlayer) entity).isCreative())) {
				...
				entity.addPotionEffect(new PotionEffect(MobEffects.WEAKNESS, this.gate * 200, (this.gate - 2) * 2));
				entity.addPotionEffect(new PotionEffect(MobEffects.SLOWNESS, this.gate * 200, this.gate - 2));
	...
	public static class RangedItem extends Item {
		...
		private final Properties GATE[] = {new Properties(0, "", 0, 0, 0, 0, 0, 0, 0, 0f, false),
			...
			new Properties(3, I18n.translateToLocal("chattext.eightgates.gate3"), 300, 20, 0x10FFFFFF, 10, 16, 1, 60, 0f, false),
				...
				Entity bullet = new EntitySmallFireball(world, attacker, x, y, z) {
					...
					public void onImpact(RayTraceResult result) {
						if (!this.world.isRemote) {
							...
							ProcedureAoeCommand bruh = ProcedureAoeCommand.set(this,0,5);
							...
							this.world.newExplosion(this.shootingEntity, this.posX, this.posY, this.posZ, 3f, false, flag);
							this.setDead();
						}
					}
					@Override
		...
		*/
					...
					if (gateOpened >= 3) {
						if (gateOpened <= 5) {
							...
							if (attacker instanceof EntityPlayer) {
								...
								entity2.sendStatusMessage(new TextComponentString(
										I18n.translateToLocalFormatted("hiddenlotus.counter",attackNum)), true);
							}
						}
						Vec3d start = attacker.getPositionVector();
						...
						if (gateOpened >= 7) {
							...
							for (int i = 0; i < 30; i++) {
								...
								Vec3d b = a.addVector(-1.5+Math.random()*3,-1.5+Math.random()*3,-1.5+Math.random()*3);
								Vec3d velocity = b.subtract(a).normalize().scale(3+Math.random()*5);
								particles.spawnParticles(Particles.Types.SMOKE,a.x, a.y, a.z, 1, 0, 0, 0,
										velocity.x, velocity.y, velocity.z, 0x00ffffff, 20);
							}
						...
						if (gateOpened == 6) {
							for (int i = 0; i < 25; i++) {
								...
								Vec3d a = start.addVector(-1.5+Math.random()*3,-1.5+Math.random()*3,-1.5+Math.random()*3);
								Vec3d velocity = look.scale(2+i/8);
								particles.spawnParticles(Particles.Types.FLAME, a.x, a.y, a.z, 1, 0, 0, 0,
										velocity.x, velocity.y, velocity.z, 0xffffcf00, 20+i*2);
							}
							particles.send();
						}
						for (int i = 1, j = 8; i <= j; i++) {
							...
							Particles.spawnParticle(attacker.world, Particles.Types.SONIC_BOOM, target.posX, target.posY+1.4d, target.posZ,
							 1, 0d, 0d, 0d, vec1.x, vec1.y, vec1.z, 0x00ffffff | ((int)((1f-(float)i/j)*0x40)<<7),
							 i * 2, (int)(5f * (1f + ((float)i/j) * 0.5f)));
						}
					}
					if (gateOpened >= 3) {
						Vec3d vec = target.getPositionVector().subtract(attacker.getPositionVector()).normalize().scale(5);
						ProcedureUtils.setVelocity(target, vec.x,vec.y,vec.z);
		...
		private float getMaxOpenableGate(ItemStack stack) {
			...
			EntityLivingBase owner = this.getOwner(stack);
			if ((owner instanceof EntityPlayer)) {
				if (PlayerTracker.getBattleXp((EntityPlayer) owner) < 15000) {
					truemax = 7;
				}
				if (PlayerTracker.getBattleXp((EntityPlayer) owner)< 10000) {
					truemax = 6;
				}
			}
			...
			for (int i = 8; i > 0; i--) {
				if (xp >= this.GATE[i].xpRequired && i <= truemax) {
					return (float)i;
				}
			}
			return 0f;
		...
		public static class GatesEntity extends Entity  {
			...
			public GatesEntity(World worldIn) {
				super(worldIn);
				this.setSize(0f, 0f);
			...
			public GatesEntity(EntityLivingBase user) {
				this(user.world);
				this.user = user;
		...
		public static class DynamicEntry extends Entity  {
			private EntityLivingBase user;
			private Vec3d start;
			private Vec3d end;
			private int dist = 35;
			...
			public DynamicEntry(World worldIn) {
				super(worldIn);
			...
			public DynamicEntry(EntityLivingBase user) {
				this(user.world);
				this.user = user;
				this.setPosition(this.user.posX, this.user.posY, this.user.posZ);
				this.playSound(SoundEvent.REGISTRY.getObject(new ResourceLocation("narutomod:dynamic_entry")),
						4f, 1f);
				this.look = this.user.getLookVec().scale(2);
			}
				...
				if (this.user != null) {
					...
					if (this.ticksExisted <= 20 && !this.hit) {
						...
						if (this.ticksExisted <= 5) {
							ProcedureUtils.setVelocity(this.user, look.x, look.y, look.z);
						}
						for (int i = 0; i < (int) 5; i++) {
							Vec3d a = this.start.addVector(-0.5+this.rand.nextFloat()*1,-0.5+this.rand.nextFloat()*1,-0.5+this.rand.nextFloat()*1);
							Vec3d b = this.look.normalize().scale(0.2+0.2*this.rand.nextFloat());
							Particles.spawnParticle(this.world, Particles.Types.SMOKE, a.x, a.y, a.z,
									1, 1d, 0d, 1d, b.x,b.y,b.z, 0xFFFFFFFF, 10, 0);
						}
						...
						for (Entity entity1 : this.world.getEntitiesWithinAABBExcludingEntity(this.user, hitbox)) {
							...
							ProcedureUtils.setVelocity(entity1, knockback.x, knockback.y, knockback.z);
		...
		public static class LeafHurricane extends Entity  {
			private EntityLivingBase user;
			private Vec3d start;
			private Vec3d end;
			private Vec3d look;
			...
			public LeafHurricane(World worldIn) {
				super(worldIn);
				this.setSize(0.01f, 0.01f);
				this.isImmuneToFire = true;
			}
			...
			public LeafHurricane(EntityLivingBase user) {
				this(user.world);
				this.user = user;
				this.setPosition(this.user.posX, this.user.posY, this.user.posZ);
				this.playSound(SoundEvent.REGISTRY.getObject(new ResourceLocation("narutomod:leaf_hurricane")),
						2f, 1f);
			}
				...
				if (this.user != null) {
					...
					if (this.ticksExisted <= 20 ) {
						...
						for (int i = 0; i < (int) 40; i++) {
							Particles.spawnParticle(this.world, Particles.Types.SMOKE, this.start.x, this.start.y+itemRand.nextFloat(), this.start.z,
									1, 1d, 0d, 1d, (itemRand.nextDouble()-0.5d) * 1.5d, 0,
									(itemRand.nextDouble()-0.5d) * 1.5d, 0xFFFFFFFF, 10, 0);
						}
						this.end = this.start.add(look.scale(1));
						...
						for (Entity entity1 : this.world.getEntitiesWithinAABBExcludingEntity(this.user, hitbox)) {
							...
							ProcedureUtils.setVelocity(entity1, knockback.x, knockback.y+1, knockback.z);
							ProcedureUtils.pushEntity(this.user,entity1,12,2);
							entity1.attackEntityFrom(ItemJutsu.causeJutsuDamage(this, this.user),damage);
		...
		public static class PrimaryLotus extends Entity  {
			private EntityLivingBase user;
			private Vec3d start;
			private Vec3d end;
			private Vec3d look;
			...
			public PrimaryLotus(World worldIn) {
				super(worldIn);
			...
			public void onUpdate() {
				...
				if (true) {
					...
					if (this.ticksExisted == 22) {
						this.playSound(SoundEvent.REGISTRY.getObject(new ResourceLocation("narutomod:primary_lotus")),
								4f, 1f);
					}
					if (this.ticksExisted > 21 && this.ticksExisted < 21+40 && !this.landed) {
						...
						for (int i = 0; i < 30; i++) {
							Particles.spawnParticle(this.user.world, Particles.Types.SMOKE, this.start.x, this.start.y, this.start.z,
									1, 1d, 0d, 1d, (itemRand.nextDouble()-0.5d) * 2.0d, 0.5d,
									(itemRand.nextDouble()-0.5d) * 2.0d, 0x10FFFFFF, 30, 0);
						}
						RayTraceResult rtr = ProcedureUtils.raytraceLook(this.user,new Vec3d(0,-1,0),5);
						...
						Vec3d tpos = this.user.getPositionVector().add(this.user.getLookVec().scale(0.75));
						this.target.setPositionAndUpdate(tpos.x,tpos.y+.5,tpos.z);
					}
		...
		public static class HiddenLotus extends Entity  {
			private EntityLivingBase user;
			private Vec3d start;
			private Vec3d end;
			private Vec3d look;
			...
			public HiddenLotus(World worldIn) {
				super(worldIn);
				this.setSize(0.001f, 0.001f);
			...
			public HiddenLotus(EntityLivingBase user, EntityLivingBase target) {
				this(user.world);
				this.user = user;
				this.target = target;
				this.setPosition(this.user.posX, this.user.posY, this.user.posZ);
				...
				for (double i = 0;i < distance; i+=0.1) {
					...
					Particles.spawnParticle(this.user.world, Particles.Types.PAPER, point.x, point.y, point.z,
							1,0,0,0, 0, 0,
							0, 0x10FFFFFF, 0, 0);
				}
			...
			public void onUpdate() {
				...
				if (true) {
					...
					if (this.ticksExisted <= 20) {
						...
						this.user.setPositionAndUpdate(this.start.x,this.start.y,this.start.z);
					}
					if (this.ticksExisted == 5) {
						this.playSound(SoundEvent.REGISTRY.getObject(new ResourceLocation("narutomod:hidden_lotus")),
								4f, 1f);
					}
					if (this.ticksExisted <= 12) {
						double distance = this.start.distanceTo(this.end);
						Vec3d targetPoint = this.start.add(this.look.scale(distance*(1-(this.ticksExisted/12))));
					...
					if (this.ticksExisted > 13 && this.ticksExisted < 13+40 && !this.landed) {
						this.target.addPotionEffect(new PotionEffect(PotionParalysis.potion, 5, 1, false, false));
						this.start = this.user.getPositionVector();

						RayTraceResult rtr = ProcedureUtils.raytraceLook(this.target,new Vec3d(0,-1,0),8);
						if (rtr.typeOfHit == RayTraceResult.Type.BLOCK) {
							landed = true;
						}
						this.end = rtr.hitVec;
						this.target.setPositionAndUpdate(this.end.x,this.end.y+3,this.end.z);
					...
					if (this.landed && !this.used) {
						this.used = true;
						boolean flag = net.minecraftforge.event.ForgeEventFactory.getMobGriefingEvent(this.world, this.user);
						Explosion explosion = this.world.newExplosion(this.user, this.end.x, this.end.y, this.end.z, 5, false, flag);
		...
		public static class AsaKujaku extends Entity  {
			private EntityLivingBase user;
			private Vec3d start;
			private Vec3d end;
			private Vec3d look;
			...
			public AsaKujaku(World worldIn) {
				super(worldIn);
				this.setSize(0.01f, 0.01f);
				this.isImmuneToFire = true;
			}
			...
			public AsaKujaku(EntityLivingBase user) {
				this(user.world);
				this.user = user;
				this.setPosition(this.user.posX, this.user.posY, this.user.posZ);
				this.playSound(SoundEvent.REGISTRY.getObject(new ResourceLocation("narutomod:asa_kujaku")),
						2f, 1f);
			}
				...
				if (this.user != null) {
					...
					if (this.ticksExisted <= 35 ) {
						if (this.ticksExisted%2== 0) {
							this.start = this.user.getPositionVector();
							this.world.playSound(null, this.start.x, this.start.y, this.start.z, net.minecraft.util.SoundEvent.REGISTRY
											.getObject(new ResourceLocation("narutomod:flamethrow")), SoundCategory.NEUTRAL,
									1f, 2f + this.rand.nextFloat() * 0.5f);
							this.look = this.user.getLookVec().scale(1);
							this.targets = new ArrayList<String>();
							Particles.Renderer particles = new Particles.Renderer(this.world);
							for (int i = 0; i < 5; i++) {
								Vec3d a = this.start.addVector(-3+this.rand.nextFloat()*6,-3+this.rand.nextFloat()*6,-3+this.rand.nextFloat()*6);
								Vec3d velocity = this.look.scale(1.5+this.rand.nextFloat()*3);
								for (int j = 0; j < 3; j++) {
									velocity = velocity.scale(1-(0.2*((double) j /3)));
									particles.spawnParticles(Particles.Types.FLAME, a.x, a.y, a.z, 1, 0, 0, 0,
											velocity.x, velocity.y, velocity.z, 0xffffcf00, 30);
								}
							}
							particles.send();
			...
			if (player.isSneaking()) {
				...
				for (int i = 0; i < (int) 40; i++) {
					Particles.spawnParticle(player.world, Particles.Types.SMOKE, player.posX, player.posY, player.posZ,
							1, 1d, 0d, 1d, (itemRand.nextDouble()-0.5d) * 0.2d, 0.001d,
							(itemRand.nextDouble()-0.5d) * 0.2d, 0x10FFFFFF, 10, 0);
				}
				if (gateOpened >= 4f) {
					for (int i = 0; i < (int) gateOpened * 10; i++) {
						Particles.spawnParticle(player.world, Particles.Types.SMOKE, player.posX, player.posY, player.posZ,
						 1, 1d, 0d, 1d, (itemRand.nextDouble()-0.5d) * 2.0d, 0.3d,
						 (itemRand.nextDouble()-0.5d) * 2.0d, 0x10FFFFFF, 30, 0);
					}
			...
			if (!world.isRemote) {
				if (entity instanceof EntityPlayer && (((EntityPlayer)entity).getHeldItemMainhand().equals(itemstack) || ((EntityPlayer)entity).getHeldItemOffhand().equals(itemstack))) {
					if (this.tpTime < 15 && this.attackTarget != null && this.attackTarget.getDistanceSq(entity) < 25d) {
						this.tpTime = 200;
						((EntityPlayer)entity).attackTargetEntityWithCurrentItem(this.attackTarget);
					}

					if (entity.getEntityData().getBoolean(NarutomodModVariables.EYETOGGLE) && !entity.onGround && this.airjump <= 0 && this.getGateOpened(itemstack) >= 3) {
						this.airjump = 60;
